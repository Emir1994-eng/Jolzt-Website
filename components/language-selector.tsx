...
export function LanguageSelector({ color = "light" }: { color?: string }) {
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" size="sm" className={`flex items-center gap-1 hover:bg-${color === "dark" ? "white/10" : "black/10"}`}>
          <GlobeIcon className="h-4 w-4" color={color === "dark" ? "#ffffff" : "#000000"} />
          <span style={{color: color === "dark" ? "#ffffff" : "#000000"}}>{currentLang.toUpperCase()}</span>
        </Button>
      </DropdownMenuTrigger>
